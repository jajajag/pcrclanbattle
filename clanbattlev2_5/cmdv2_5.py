...
MONTH_ZODIAC = {
        1: '♒水瓶座', 2: '♓双鱼座', 3: '♈白羊座', 4: '♉金牛座',
        5: '♊双子座', 6: '♊双子座', 7: '♌狮子座', 8: '♍处女座',
        9: '♎天秤座', 10: '♏天蝎座', 11: '♐射手座', 12: '♑摩羯座'}
...
def _gen_progress_text(bm):
    clan = _check_clan(bm)
    status = bm.get_challenge_progress(1)
    # Find zodiac for current month
    month = datetime.now().month
    month = (month - 5) % 12 if clan['server'] == bm.SERVER_TW else month
    text = MONTH_ZODIAC[month]
    # Generate text for output
    min_round = min([status[i][0] for i in range(1, 6)])
    stage = bm.get_stage(min_round, clan['server'])
    text += ' ' + str(min_round) + '周目 ' + str(stage) + '阶段'
    for i in range(1, 6):
        round_, hp = status[i]
...
async def process_challenge(bot:NoneBot, ctx:Context_T, ch:ParseResult):
    ...
    cur_round, cur_boss, cur_hp = bm.get_challenge_progress(1)
    ...
    aft_round, aft_boss, aft_hp = bm.get_challenge_progress(1)

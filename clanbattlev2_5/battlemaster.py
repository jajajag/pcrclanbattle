...
class BattleMaster(object):
    ...
    def add_clan(self, cid, name, server):
        config = self.config
        boss_hp = [config[config['BOSS_HP'][server]][0][i] for i in range(5)]
        # Add clan with initialized boss hp
        return self.clandao.add(
                {'gid': self.group, 'cid': cid, 'name': name, 'server': server,
                    'hp': boss_hp})
    ...
    def add_member(self, uid, alt, name, cid):
        return self.memberdao.add(
                {'uid': uid, 'alt': alt, 'name': name,
                    'gid': self.group, 'cid': cid})
    ...
    def mod_member(self, uid, alt, new_name, new_cid):
        return self.memberdao.modify(
                {'uid': uid, 'alt': alt, 'name': new_name,
                    'gid': self.group, 'cid': new_cid})
    ...
    def get_challenge_progress(self, cid):
        '''
        return {boss: (round, remain_hp)}
        '''
        clan = self.get_clan(cid)
        if not clan:
            return None
        # Return status defined in clan
        ret = {}
        for i in range(5):
            ret[i + 1] = (clan['round' + str(i + 1)], clan['hp' + str(i + 1)])
        return ret

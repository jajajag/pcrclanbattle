...
class ClanDao(SqliteDao):
    ...
    def __init__(self):
        super().__init__(
            ...
            PRIMARY KEY (gid, cid)
            ''')
        # We also save round and hp for 5 bosses


    @staticmethod
    def row2item(r):
        return {
                'gid': r[0], 'cid': r[1], 'name': r[2], 'server': r[3],
                'round1': r[4], 'round2': r[5], 'round3': r[6],
                'round4': r[7], 'round5': r[8],
                'hp1': r[9], 'hp2': r[10], 'hp3': r[11],
    ...
    # We only update name and server type using this function 
    ...
    # Update boss round and hp with update function
    def update(self, boss, round_, hp):
        with self._connect() as conn:
            try:
                conn.execute('''
                    UPDATE {0} SET round{1}=?, hp{1}=? WHERE gid=? AND cid=?
                    '''.format(self._table, str(boss)),
                    (round_, hp, clan['gid'], clan['cid']))
            except (sqlite3.DatabaseError) as e:
                logger.error(f'[ClanDao.update] {e}')
                raise DatabaseError('修改状态失败')       
...
class MemberDao(SqliteDao):
    ...
    @staticmethod
    def row2item(r):
        return {'uid': r[0], 'alt': r[1], 'name': r[2],
                'gid': r[3], 'cid': r[4]} if r else None
